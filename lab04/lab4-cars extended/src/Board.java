    ...
    private void initialize(int length, int height) {
        ...
        for (int x = 0; x < points.length; ++x) {
            for (int y = 0; y < points[x].length; ++y) {
                if (y == (points[x].length) / 2 - 1) {
                    points[x][y] = new Point(0, "left");
                } else if (y == (points[x].length) / 2) {
                    points[x][y] = new Point(0, "right");
                } else {
                    points[x][y] = new Point(5);
                }
            }
        }
        ...
        if ((x < points.length) && (x > 0) && (y < points[x].length) && (y > 0)) {
            if (editType == 0) {
                ...
                switch (points[x][y].type){
                    case 1 -> points[x][y].speed = points[x][y].max_speed = 3;
                    case 2 -> points[x][y].speed = points[x][y].max_speed = 5;
                    case 3 -> points[x][y].speed = points[x][y].max_speed = 7;
                }
            }
            this.repaint();
        }
